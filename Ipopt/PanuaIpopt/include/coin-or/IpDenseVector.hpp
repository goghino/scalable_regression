...
protected:
   ...
   virtual void ElementWiseFuncImpl(Number (*func)(Number));
...
private:
   ...
   /** Dense Number array of vector values.  This is mutable for the
       case that the vector is currently a homogenous vector but the
       array is requested for the const case.  Then, we are allocating
       or setting the values but keep the vector homogeneous. */
   mutable Number* values_;
   ...
   inline Number* values_allocated() const;
   ...
   /** Auxiliary method for setting explicitly all elements in values_
    * to the current scalar value.  The const version is used if the
    * array is requested for a homogeneous const vector.
    */
   void set_values_from_scalar() const;
};
...
inline const Number* DenseVector::Values() const
{
   ...
   if( homogeneous_ )
   {
      // If currently the vector is a homogeneous vector, set all
      // elements explicitly to this value.  This is a const function,
      // but the array is mutable.
      set_values_from_scalar();
   }
   return values_;
}

inline Number* DenseVector::values_allocated() const
{
   if( values_ == NULL )
   {
